...
private:
    ...
    void QueueAutoExchange(Player* player, uint32 tokenItemId, bool queueAllStagedTokens, bool logWhenEmpty, char const* reason, uint32 delayMs = 0);
    void QueueWotlkAutoExchange(Player* player, uint32 tokenItemId, bool queueAllStagedTokens, bool logWhenEmpty, char const* reason, uint32 delayMs = 0);
    void ProcessAutoExchangeQueue(Player* player);
    void ProcessWotlkAutoExchangeQueue(Player* player);
    bool CanRetryAutoExchangeToken(Player* player, uint32 tokenItemId);
    bool CanRetryWotlkAutoExchangeToken(Player* player, uint32 tokenItemId);
    ...
    bool AreAllLootCandidatesUnneeded(Player* player, uint32 tokenItemId) const;
    ...
    void ExchangePlayerTokens(Player* player, char const* label, uint32 maxPerBotPerPass = 0, class ChatHandler* handler = nullptr, std::unordered_set<uint32> const* tokenFilter = nullptr);
    void ExchangeWotlkPlayerTokens(Player* player, char const* label, class ChatHandler* handler = nullptr, uint32 maxPerBotPerPass = 0, std::unordered_set<uint32> const* tokenFilter = nullptr);
    ...
    uint32 _autoExchangeRetryDelayMs = 30000;
